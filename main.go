		...
		if serverIndex == "1" {
			...
			for _, filename := range strlist {
				...
				case "linux":
					...
					//通过可执行权限位判断，排除目录和安装程序自身
					if hw.Mode().Perm()&0111 != 0 && !hw.IsDir() && !strings.EqualFold(hw.Name(), "installserver") && !strings.EqualFold(hw.Name(), "install-server-arm64") && !strings.EqualFold(hw.Name(), "install-server-amd64") {
						exeName[i] = hw.Name()
						i++
					}
				case "windows":
